...
iso_phase = Phase.from_preset("default_cubic")
...
class TestIsotropicStiffnessGenerator:
    ...
    def test_generate_with_identity_orientations(self):
        ...
        texture = Texture(
            orientations=orientations, representation="rotmat", phase=iso_phase
        )
        ...
        for i in range(n):
            np.testing.assert_array_almost_equal(
                stiffness.stiffness_tensors[i], gen._base_tensor
            )
    ...
    def test_generate_converts_euler_to_rotmat(self):
        ...
        for i in range(n):
            np.testing.assert_array_almost_equal(
                stiffness.stiffness_tensors[i], gen._base_tensor, decimal=10
            )
    ...
    def test_single_grain(self):
        ...
        np.testing.assert_array_almost_equal(
            stiffness.stiffness_tensors[0], gen._base_tensor
        )
    ...
    def test_90_degree_rotation_symmetry(self):
        ...
        np.testing.assert_array_almost_equal(
            stiffness.stiffness_tensors[0], gen._base_tensor, decimal=10
        )
