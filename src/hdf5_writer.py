...
def write_d3d(microstructure, filename):
    ...
    with h5py.File(filename, 'w') as f:
        ...
        f.attrs.create('DREAM3D Version', '6.5.0', dtype='S10')  # Or your installed version
        ...
        data_container.attrs['AttributeMatrixNames'] = ['CellData', 'CellFeatureData', 'CellEnsembleData']
        ...
        if len(microstructure.dimensions) == 3:
            dims = np.array([microstructure.dimensions[2], 
                           microstructure.dimensions[1], 
                           microstructure.dimensions[0]], dtype=np.uint64)
        else:
            # Handle 2D case
            dims = np.array([microstructure.dimensions[1], 
        ...
        spacing = np.array([microstructure.resolution, 
                          microstructure.resolution, 
                          microstructure.resolution], dtype=np.float32)
        ...
        num_features = max(microstructure.orientations.keys()) if microstructure.orientations else 0
        ...
        phases_feature_dataset = feature_data.create_dataset('Phases', data=feature_phases)
        ...
        crystal_dataset = ensemble_data.create_dataset('CrystalStructures', data=crystal_structures)
...
def write_raw_binary(microstructure, filename_prefix):
    ...
    with open(meta_filename, 'w') as f:
        f.write(f"Dimensions: {microstructure.dimensions[0]} {microstructure.dimensions[1]} {microstructure.dimensions[2]}\n")
        f.write(f"Resolution: {microstructure.resolution}\n")
        f.write(f"Data Type: int32\n")
        f.write(f"Byte Order: lttle-endian\n")
...
def write_hdf5(microstructure, filename):
    ...
    with h5py.File(filename, 'w') as f:
        ...
        grain_ids_dataset = f.create_dataset('GrainIDs', 
                                            data=microstructure.grain_ids,
                                            compression='gzip')
        ...
        num_grains = max(microstructure.orientations.keys()) if microstructure.orientations else 0
        ...
        euler_dataset.attrs['description'] = 'Euler angles (Bunge ZXZ convention) in radians'
        ...
        print(f"  1. Use filter: 'Import HDF5 Dataset'")
        print(f"  2. Select dataset: '/GrainIDs'")
