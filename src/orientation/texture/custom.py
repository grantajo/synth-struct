...
class CustomTexture(TextureGenerator):
    ...
    def __init__(self, hkl, uvw, degspread=5.0, seed=None):
        ...
        if dot_product > 1e-6:
            ...
            warnings.warn(f"uvw was not orthogonal to hkl (dot product = {dot_product:.4f}). "
    ...
    def _apply_scatter(self, base_euler):
        ...
        perturbations = np.random.normal(0., self.degspread, size=(n, 3))
        
        orientations = base_euler, + perturbations
        
        orientations[:, 0] %= 360.
        orientations[:, 1] %= np.clip(orientations[:, 1], 0., 180.)
        orientations[:, 2] %= 360.
