...
class LathGenerator(MicrostructureGenerator):
    ...
    def __init__(
        ...
        self.num_colonies = (
            num_colonies if num_colonies is not None else max(1, num_grains // 8)
        )
    ...
    def _generate_internal(self, micro):
        ...
        self.is_basketweave = (
            np.random.rand(self.num_grains) < self.basketweave_fraction
        )
        ...
        aniso_voronoi_assignment(
            micro, self.seeds, self.scale_factors, self.rotations, self.chunk_size
        )
        ...
        print(
            f"Generated {self.num_grains} laths in {self.num_colonies} colonies "
    ...
    def _generate_colony_structure(self, dimensions):
        ...
        self.colony_centers = np.random.rand(self.num_colonies, 3) * np.array(
            dimensions
        )
    ...
    def _generate_lath_params(self):
        ...
        for i in range(self.num_grains):
            ...
            aspect_ratio = np.random.normal(
                self.aspect_ratio_mean, self.aspect_ratio_std
            )
            ...
            misori = np.random.uniform(
                -self.colony_misorientation, self.colony_misorientation, 3
            )
    ...
    def _get_variant_orientations(self):
        ...
        if self.bw_variants == 2:
            # Two variants
            return np.array([[0, np.pi / 4, 0], [np.pi / 2, np.pi / 4, 0]])
