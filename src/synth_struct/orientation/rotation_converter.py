...
def euler_to_rotation_matrix(euler_angles):
    ...
    # Second row
    R[:, 1, 0] = -np.cos(phi1) * np.sin(phi2) - np.sin(phi1) * np.cos(phi2) * np.cos(
        Phi
    )
    R[:, 1, 1] = -np.sin(phi1) * np.sin(phi2) + np.cos(phi1) * np.cos(phi2) * np.cos(
        Phi
    )
...
def quat_to_euler(quats):
    ...
    mask1 = (chi == 0) & (q12 == 0)
    euler_angles[mask1, 0] = np.arctan2(
        -2 * q0[mask1] * q3[mask1], q0[mask1] ** 2 - q3[mask1] ** 2
    )
    euler_angles[mask1, 1] = 0.0
    euler_angles[mask1, 2] = 0.0
    ...
    # Case 3: General case
    mask3 = ~mask1 & ~mask2
    euler_angles[mask3, 0] = np.arctan2(
        (q1[mask3] * q3[mask3] - q0[mask3] * q2[mask3]) / chi[mask3],
        (-q0[mask3] * q1[mask3] - q2[mask3] * q3[mask3]) / chi[mask3],
    )
    euler_angles[mask3, 1] = np.arctan2(2 * chi[mask3], q03[mask3] - q12[mask3])
    euler_angles[mask3, 2] = np.arctan2(
        (q0[mask3] * q2[mask3] + q1[mask3] * q3[mask3]) / chi[mask3],
...
def rotation_matrix_to_quat(R):
    ...
    quats[:, 0] = 0.5 * np.sqrt(
        np.clip(1 + R[:, 0, 0] + R[:, 1, 1] + R[:, 2, 2], 0, None)
    )
    quats[:, 1] = 0.5 * np.sqrt(
        np.clip(1 + R[:, 0, 0] - R[:, 1, 1] - R[:, 2, 2], 0, None)
    )
