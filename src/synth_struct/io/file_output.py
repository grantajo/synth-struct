...
def export_microstructure(
    ...
    if target_frequency_hz is not None:
        _validate_element_size(
            micro, target_frequency_hz, wave_velocity,
        )
...
def _build_conforming_mesh(
    ...
    return _gmsh_conforming_tet(
        surface_meshes, micro, element_type, periodic_bc,
    )
...
def _convex_region_to_clipped_mesh(
    ...
    try:
        ...
        faces_pv = np.hstack([
            np.full((len(hull.simplices), 1), 3), 
            hull.simplices
        ]).astype(np.int32)

        mesh = pv.PolyData(verts, faces_pv)
        box = pv.Box(
            bounds=[0, bounds[0], 0, bounds[1], 0, bounds[2] if is_3d else 1]
        )
...
def _gmsh_conforming_tet(
    ...
    # Save to temp file and load into PyVista
...
def _apply_gmsh_periodic_bc(micro):
    ...
    for normal, length in face_pairs:
        translation = [
            1, 0, 0, normal[0] * length,
            0, 1, 0, normal[1] * length,
            0, 0, 1, normal[2] * length,
            0, 0, 0, 1
        ]
...
def _write_format(
    ...
    elif solver_format in (SolverFormat.ABAQUS_STANDARD,
                           SolverFormat.ABAQUS_EXPLICIT):
        _write_abaqus_inp(mesh, micro, filepath,
                          explicit=(solver_format == SolverFormat.ABAQUS_EXPLICIT),
                          periodic_bc=periodic_bc)

    elif solver_format == SolverFormat.FENICS:
        mesh.save(filepath)   # PyVista writes .msh directly
...
def _write_abaqus_inp(
    ...
    with open(filepath, "w") as f:
        f.write("*Heading\n")
        f.write(f" Microstructure export — {'Explicit' if explicit else 'Standard'}\n\n")
        ...
        for name, mask_fn in face_defs.items():
            ...
            if len(node_ids):
                f.write(f"\n*Nset, nset={name}\n")
                for chunk in [node_ids[i:i+16] for i in range(0, len(node_ids), 16)]:
                    f.write("  " + ", ".join(map(str, chunk)) + "\n")
    ...
    with h5py.File(filepath, "w") as f:
        ...
        if micro.orientations is not None:
            grains_grp.create_dataset(
                "orientations",
                data=micro.orientations[:n].astype(np.float64)
            )
        ...
        if hasattr(micro, 'stiffness') and micro.stiffness is not None:
            grains_grp.create_dataset(
                "stiffness",
                data=micro.stiffness[:n].astype(np.float64)
            )
        ...
        if hasattr(micro, 'phase') and micro.phase is not None:
            grains_grp.create_dataset(
                "phase_id",
                data=micro.phase[:n].astype(np.int32)
            )
...
def _validate_element_size(
    ...
    print(f"  Wave check: f={freq_mhz:.1f} MHz, λ={wavelength*1e3:.2f} mm, "
