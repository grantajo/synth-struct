# synth_struct/src/plotting/gen_plot.py

import numpy as np
import matplotlib.pyplot as plt
from matplotlib.colors import ListedColormap
from .plot_utils import shuffle_display_grain_ids, get_colony_colormap, create_grain_boundary_overlay
...
class Plotter:
    ...
    @staticmethod
    def plot_grain_ids(ax, micro, shuffle=False, seed=None,
                       cmap='nipy_spectral', show_boundaries=False,
                       boundary_color='black', boundary_width=0.5,
                       slice_direction='z', slice_index=None,
        ...
        if shuffle:
            grain_ids = shuffle_display_grain_ids(micro.grain_ids, micro.num_grains, seed=seed)
        ...
        if grain_ids.ndim == 3:
            ...
            if slice_direction == 'z':
                ...
                raise ValueError(f"slice_direction must be 'x', 'y', or 'z', got '{slice_direction}'")
    ...
    @staticmethod
    def plot_3d_slices(fig, micro, slice_indices=None, shuffle=False,
                       seed=None, cmap='nipy_spectral', show_boundaries=False,
    ...
    @staticmethod
    def plot_grain_size_dist(ax, micro, bins=50, log_scale=False, 
                             title='Grain Size Distribution', **kwargs):
        """
        ...
        unique_ids, counts = np.unique(micro.grain_ids[micro.grain_ids > 0], return_counts=True)
        ...
        ax.axvline(median_size, color='blue', linestyle='--', label=f'Median: {median_size:.1f}')
        ax.legend()
