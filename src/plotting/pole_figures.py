# synth_struct/src/plotting/pole_figures.py
...
def plot_pole_figure(ax, micro, miller_index, crystal_structure='cubic', grain_subset=None,
                     show_labesl=True, sample_fraction=None, marker_size=1, **scatter_kwargs):
    """
    ...
    **scatter_kwargs: Additional elements passed to ax.scatter (e.g., c='red', alpha=0.5, cmap='viridis')
    ...
    h, k, l = miller_index
    ax.set_title(fr"$\{{{h}\,{k}\,{l}\}}$ Pole Figure")
    
    return pf
    
    
def plot_multiple_pole_figures(axes, micro, miller_indices, crystal_structure='cubic'
                               grain_subset=None, show_labels=True, sample_fraction=None,
                               **plot_kwargs):
    """
...
def create_pole_figure_axes(fig, n_figures, projection='stereographic', layout='row'):
    ...
    if layout == 'row':
        ...
        raise ValueError(
            f"Layout must be 'row', 'column' or 'grid', got '{layout}'"
        )
...
def create_standard_pole_figures(micro, crystal_structure='cubic',
